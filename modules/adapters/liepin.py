...
def fetch_liepin(urls: List[str], cookie: str) -> List[Dict]:
    logger = get_logger("liepin")
    sess = requests.Session()
    headers = {"User-Agent": "Mozilla/5.0", "Referer": "https://www.liepin.com/"}
    ...
    for u in urls or []:
        try:
            ...
            if r.status_code != 200:
                logger.info(f"liepin_fail status={r.status_code} url={u}")
                continue
            ...
            logger.info(f"liepin_ingested url_len={len(u)} text_len={len(cleaned)}")
        except Exception:
            logger.info(f"liepin_error url={u}")
    return out