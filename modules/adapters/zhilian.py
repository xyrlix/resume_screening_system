...
def fetch_zhilian(urls: List[str], cookie: str) -> List[Dict]:
    logger = get_logger("zhilian")
    sess = requests.Session()
    headers = {"User-Agent": "Mozilla/5.0", "Referer": "https://www.zhaopin.com/"}
    ...
    for u in urls or []:
        try:
            ...
            if r.status_code != 200:
                logger.info(f"zhilian_fail status={r.status_code} url={u}")
                continue
            ...
            logger.info(f"zhilian_ingested url_len={len(u)} text_len={len(cleaned)}")
        except Exception:
            logger.info(f"zhilian_error url={u}")
    return out